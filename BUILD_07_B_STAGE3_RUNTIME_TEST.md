...
1. In top orthographic view create a default 3600 mm straight Stair. Confirm `STANDARD_RESIDENTIAL`, schema `>=2`, both booleans true, profile width 40 mm, Y thickness 18 mm, and compatibility body closure depth 150 mm in Console.
2. Confirm the complete world-Y envelope remains the 900 mm stair width:
   ```python
   print(round(min(v.co.y for v in M.vertices),6),round(max(v.co.y for v in M.vertices),6))
   ```
   For an X-axis Stair the complete envelope must print `-0.45 0.45`. Inspect front, top, and oblique views and confirm neither board protrudes beyond the 900 mm body.
...
4. For LEFT-only and RIGHT-only verify the Path center did not move and complete envelopes remain `[-0.450,+0.450]`. Isolate board vertices and verify LEFT `[+0.432,+0.450]`, RIGHT `[-0.450,-0.432]`; confirm Y thickness is 18 mm.
...
6. Set Side Board profile width from 40 to 60 mm and confirm only the thin stepped finish strip grows: the body soffit, first-step bottom, and all Tread/Riser/Underbody vertices remain unchanged. Restore profile width to 40 mm. The internal compatibility closure-depth field remains 150 mm. From side orthographic view confirm the strip is above/downhill of the step contour: vertical offsets go toward `-X`, horizontal offsets toward `+Z`. It must not resemble Candidate r1’s large under-stair plate. Then set Side Board thickness to 24 mm and verify only the external Y thickness changes; restore 18 mm.
...
5. Test atomic rejection: enter an invalid Side Board profile width (`>= min(h,g)`) through redo/operator input. Confirm operation cancels and old mesh pointer, dimensions, Path, ascent, mode/schema, fields/material references, slots, ID, and transform are unchanged.
